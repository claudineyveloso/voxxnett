...
export const createSession = async (email, password, user_name, user_type) => {
  return await api.post(
     '/signup', 
    { "user": { "email": email, "password": password, "user_name": user_name, "user_type": user_type }}
  ).then((response) => {
    return response;
  })
  .catch(function(err) {
...
export const getUsers = async (searchUser) => {
  return await api.get(`/api/v1/users/${searchUser}`)
    .then((response) => {
      return response;
  }).catch((err) => {
...
export const createUser = async (objectUser, token) => {
  return await api.post(`/api/v1/users/custom_create`, JSON.stringify(objectUser), { "Authorization": `Bearer ${token}`})
    .then((response) => {
      return response;
    }).catch((err) => {
